 ...
 *   - Mailpit (https://mailpit.axllent.org) — local SMTP + UI
 */
class EmailService {
  ...
  /**
   * Verify that the SMTP transport is reachable.
   * Called during server startup — failure is non-fatal (just logs a warning).
   */
  async verifyConnection(): Promise<void> {
    try {
      await this.transporter.verify();
      logDebug("✅ Email service is ready.");
    } catch (error) {
  ...
  private async sendMail(
    ...
    try {
      ...
      // Log preview URL when using Ethereal / test accounts
      const previewUrl = nodemailer.getTestMessageUrl(info);
      if (previewUrl) logDebug(`🔗 Preview: ${previewUrl}`);
  ...
  async sendWelcomeEmail(to: string, userName: string): Promise<EmailServiceResponse> {
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>Your account has been created successfully. We're glad to have you.</p>
